...
from .models import ComentarioDocumento, ComentarioFicha
...
class StudentGeneralForm(forms.Form):
    ...
    direccion_actual = forms.CharField(
        required=False,
        widget=forms.Textarea,
    )
    direccion_origen = forms.CharField(
        required=False,
        widget=forms.Textarea,
    )
    ...
    # Se mapea a 'seguro' en el modelo
    prevision = forms.CharField(required=False, max_length=20)
...
class StudentVaccinesForm(forms.Form):
    ...
    influenza_fecha = forms.DateField(
        required=False,
        input_formats=["%Y-%m-%d"],
    )
